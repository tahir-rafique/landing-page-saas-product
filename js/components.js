...
/* -------------------------------------------
   ...
   Reusable button factory used across the page.
   Variants: 'primary' | 'dark' | 'outline' | 'text'
   Returns an <a> for href, or <button> for type.
   ------------------------------------------- */
...
/* -------------------------------------------
   Hydrate buttons placed via data attributes.
   Any element with [data-button] in markup is
   replaced with a real button rendered by
   createButton(). Keeps HTML clean and
   ensures consistency across the page.
   ------------------------------------------- */
function hydrateButtons(root = document) {
  root.querySelectorAll("[data-button]").forEach((node) => {
    ...
    // Preserve form submission semantics if inside a form
    node.replaceWith(btn);
  });
}

/* -------------------------------------------
   Service illustrations — small inline SVGs
   ------------------------------------------- */
const SERVICE_ILLUSTRATIONS = {
  ...
  social: `<svg viewBox="0 0 130 130" xmlns="http://www.w3.org/2000/svg"><circle cx="40" cy="55" r="22" fill="#191A23"/><circle cx="85" cy="80" r="22" fill="#B9FF66" stroke="#191A23" stroke-width="3"/><path d="M30 50 q5 -8 12 -3" stroke="#B9FF66" stroke-width="3" fill="none"/><path d="M76 78 q5 5 12 0" stroke="#191A23" stroke-width="3" fill="none"/><rect x="22" y="92" width="38" height="6" rx="3" fill="#191A23"/></svg>`,
...
function renderServices(target, services) {
  ...
  services.forEach((s) => {
    ...
    // Title with each line wrapped in a highlight span
    const titleHtml = s.title
      .map((line) => `<span>${line}</span>`)
      .join("<br/>");

    const arrowColor = s.theme === "dark" ? "#191A23" : "#B9FF66";
  ...
  studies.forEach((c) => {
    ...
    card.innerHTML = `
      ...
      <a href="#contact" class="case-link">
        Learn more
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M7 17L17 7M9 7h8v8"/></svg>
      </a>
    `;
    target.appendChild(card);
  });
...
/* -------------------------------------------
   Render Working Process accordion
   ------------------------------------------- */
    ...
    item.innerHTML = `
      <button class="process-trigger" aria-expanded="${idx === 0}">
        ...
        <span class="process-toggle" aria-hidden="true">
          <svg class="icon-plus" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#191A23" stroke-width="2" stroke-linecap="round"><path d="M12 5v14M5 12h14"/></svg>
        </span>
      </button>
...
function teamPhotoSvg(initials) {
  return `
    <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="50" y="46" text-anchor="middle" fill="#B9FF66" font-family="Space Grotesk" font-size="18" font-weight="600">${initials}</text>
    </svg>
  `;
}
...
function renderTeam(target, members) {
  ...
  members.forEach((m) => {
    ...
    card.innerHTML = `
      <div class="team-head">
        <div class="team-photo">${teamPhotoSvg(getInitials(m.name))}</div>
        <div>
